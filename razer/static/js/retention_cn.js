...
$(document).ready(function() {
    $(".range-select").select2({
        ...
        if($(this).val() === "events" || $(this).val() === "事件") {
            $(".range-others").show();
        } else {
            $(".range-others").hide();
        }
...
function updateEventLabelNumbering() {
    ...
    $userLabels.each(function(i) { $(this).html("事件 " + ++i); });
    $cohortLabels.each(function(i) { $(this).html("事件 " + ++i); });
}
...
function buildQuery(){
	...
	for(var h = 0; h < eventSelectSize; h++){
		...
		if(eventDiv.find('.adv').is(":checked")){
			// console.log("Advance Enabled.")
			var v1 = parseInt(eventDiv.find('.adv1').val());
			var v2 = parseInt(eventDiv.find('.adv2').val());
			var v3 = parseInt(eventDiv.find('.adv3').val());
			eventSelect['cohortFields'][0]['minLevel'] = Number.isInteger(v1)?v1:0;
			eventSelect['cohortFields'][0]['numLevel'] = Number.isInteger(v2)?v2:10;
			eventSelect['cohortFields'][0]['scale'] = Number.isInteger(v3)?v3:20;
